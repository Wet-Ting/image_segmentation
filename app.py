...
from ip_gui import Ui_MainWindow
import cv2 as cv
import numpy as np  
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def run(self):
        ...
        imgNum = self.textEdit.toPlainText()
        model = self.textEdit_2.toPlainText()
        
        if int(imgNum) <= 60:
            in_files = 'data/f0' + str(math.ceil(int(imgNum) / 20)) + '/image/00' + imgNum + '.png'
            gt = 'data/f0' + str(math.ceil(int(imgNum) / 20)) + '/label/00' + imgNum + '.png'
            out_files = 'output_gui/00' + imgNum + '.png'
        elif int(imgNum) > 60:
            in_files = 'data/other/image/00' + imgNum + '.png'
            gt = 'data/other/label/00' + imgNum + '.png'
            out_files = 'output_gui/00' + imgNum + '.png'
       
        net = UNet_ver2(1).to(device) 
        net.load_state_dict(torch.load('ckpt_0' + model + '_ver5/best.pth'))
